  ...
  useEffect(() => {
    ...
    const fetchDropdownOptions = async () => {
      try {
        ...
        const techniciansRes = await labService.getAllLabTechnicians();
        console.log('Lab Technicians:', techniciansRes.data);
        ...
        setLabTechnicians(techniciansRes.data || []);
        ...
        {activeTab === 'lab-tests' && (
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <form onSubmit={async (e) => {
                ...
                if (!labTestForm.technicianId) {
                  setError('Please select a lab technician');
                  return;
                }
                ...
                try {
                  ...
                  const request = {
                    ...
                    technicianId: parseInt(labTestForm.technicianId),
                ...
                <div>
                  <label style={{ fontWeight: 'normal' }} className="block text-sm mb-2">Lab Technician</label>
                  <select
                      value={labTestForm.technicianId}
                      onChange={(e) => setLabTestForm({ ...labTestForm, technicianId: e.target.value })}
                      className="w-full px-4 py-2 border rounded-lg"
                      required
                  >
