...
public interface PerformedProcedureRepository extends JpaRepository<PerformedProcedures, Long> {
    ...
    // performedId is NOT auto-generated by the database (supplied by the application),
    // so callers must compute the next free ID before saving a new PerformedProcedures.
    @Query("SELECT COALESCE(MAX(e.performedId), 0) FROM PerformedProcedures e")
    Long findMaxPerformedId();
