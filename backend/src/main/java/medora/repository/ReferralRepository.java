...
public interface ReferralRepository extends JpaRepository<Referrals, Long> {
    ...
    // referralId is NOT auto-generated by the database (supplied by the application),
    // so callers must compute the next free ID before saving a new Referrals.
    @Query("SELECT COALESCE(MAX(e.referralId), 0) FROM Referrals e")
    Long findMaxReferralId();
