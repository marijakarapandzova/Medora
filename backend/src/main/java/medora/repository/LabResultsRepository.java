...
public interface LabResultsRepository extends JpaRepository<LabResults, Long> {
    ...
    // resultId is NOT auto-generated by the database (supplied by the application),
    // so callers must compute the next free ID before saving a new LabResults.
    @Query("SELECT COALESCE(MAX(e.resultId), 0) FROM LabResults e")
