...
public interface PrescriptionRepository extends JpaRepository<Prescriptions, Long> {
    ...
    // prescriptionId is NOT auto-generated by the database (supplied by the application),
    // so callers must compute the next free ID before saving a new Prescriptions.
    @Query("SELECT COALESCE(MAX(e.prescriptionId), 0) FROM Prescriptions e")
    Long findMaxPrescriptionId();
