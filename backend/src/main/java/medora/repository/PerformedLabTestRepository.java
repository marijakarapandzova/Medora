...
public interface PerformedLabTestRepository extends JpaRepository<PerformedLabTests, Long> {
    ...
    // performedTestId is NOT auto-generated by the database (supplied by the application),
    // so callers must compute the next free ID before saving a new PerformedLabTests.
    @Query("SELECT COALESCE(MAX(e.performedTestId), 0) FROM PerformedLabTests e")
    Long findMaxPerformedTestId();
