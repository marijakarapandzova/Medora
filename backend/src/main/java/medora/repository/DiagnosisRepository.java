...
public interface DiagnosisRepository extends JpaRepository<Diagnosis, Long> {
    ...
    // diagnosis_id is NOT auto-generated by the database (supplied by the application),
    // so callers must compute the next free ID before saving a new Diagnosis.
    @Query("SELECT COALESCE(MAX(d.diagnosisId), 0) FROM Diagnosis d")
    Long findMaxDiagnosisId();
